	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err := client.Start(); err != nil {
				fmt.Println(errors.Wrap(err, "error during flare.Client start"))
				os.Exit(1)
			}
			...
			if err := client.Stop(); err != nil {
				fmt.Println(errors.Wrap(err, "error during flare.Client stop"))
				os.Exit(1)
			}
		},
	}
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err := client.Setup(ctx); err != nil {
				fmt.Println(errors.Wrap(err, "error during flare.Client setup"))
				os.Exit(1)
			}
		},
	}
...
func newClient(path string) *flare.Client {
	...
	client := &flare.Client{Config: config}
