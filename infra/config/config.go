...
// UnknowEntries check if the config has any key that is not expected.
func (c *Client) UnknowEntries(keys []string) []string {
	var unknow []string

	for _, ckey := range c.viper.AllKeys() {
		var found bool
